...
class TranslateOptions(BaseOptions):
    def __init__(self, parent: QWidget):
        super().__init__(parent, [TOGGLE_CHOICES, TRANSLATE_MODEL, TOGGLE_CHOICES])
    ...
    def updateDisplay(self):
        ...
        if translateModelName == "ArgosTranslate":
            ...
            self.translateModel = "ArgosTranslate"
        elif translateModelName == "ChatGPT" or translateModelName == "DeepL":
            self.apiLabel.show()
            self.apiLineEdit.show()
            if translateModelName == "ChatGPT":
                self.translateModel = "ChatGPT"
            else:
                self.translateModel = "DeepL"
        else:
            self.apiLabel.hide()
            self.apiLineEdit.hide()
