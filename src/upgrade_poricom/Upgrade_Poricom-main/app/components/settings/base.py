...
class BaseSettings(QWidget):
    ...
    def saveSettings(self, hasMessage=True):
        print('(settings)..base.py saveSettings\n')
        print(hasMessage)
        for propName, _ in self._defaults.items():
            self.settings.setValue(
                f"{self._prefix}{propName}", self.getProperty(propName)
            )
            print(f"{self._prefix}{propName}", self.getProperty(propName))
            print('settings ..base.py success!')
            print(hasMessage)
        if hasMessage:
            print('(settings)..base.py BasePopup')
